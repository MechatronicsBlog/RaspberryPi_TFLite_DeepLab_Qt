...
bool ObjectsRecogFilter::getShowBackground() const
{
    return background;
}

void ObjectsRecogFilter::setShowBackground(bool value)
{
    background = value;
    tf.setShowBackground(background);
}

bool ObjectsRecogFilter::getSemiTransparent() const
{
    return transparent;
}

void ObjectsRecogFilter::setSemiTransparent(bool value)
{
    transparent = value;
    tf.setSemiTransparent(transparent);
}
...
QVideoFrame ObjectsRecogFilterRunable::run(QVideoFrame *input, const QVideoSurfaceFormat &surfaceFormat, RunFlags flags)
{
    ...
    {       
        ...
        if (img.format() != QImage::Format_Invalid)
        {
            ...
            if (network == TensorFlow::knIMAGE_SEGMENTATION)
            {
                // Assumed only 1 masks that must be scaled to the image size
                QImage segmentation = masks.first();

                img = AuxUtils::drawSegmentation(img,segmentation);
